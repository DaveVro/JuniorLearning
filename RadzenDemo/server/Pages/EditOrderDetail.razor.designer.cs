...
namespace RadzenDemo.Pages
{
    public partial class EditOrderDetailComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Form0Submit(RadzenDemo.Models.Sample.OrderDetail args)
        {
            ...
            catch (System.Exception sampleUpdateOrderDetailException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update OrderDetail" });
            }
        }
